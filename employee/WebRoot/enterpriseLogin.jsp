...
<!--[if lt IE 9]>
<script type="text/javascript"  src="/pcend/public/js/libs/compatible.js"></script>
<![endif]-->

    <title>企业登录</title>
    <style>
        a:hover{color:white;text-decoration: none;}
        .qq-help a:hover{color:#337ab7;text-decoration: none;}
    </style>
    ...
    <script>
        ...
        if(_url.indexOf('222.204.6.143') > 0 && _url.indexOf('school_token') < 0){
            location.href = "http://222.204.6.143:8087/login/index.html?school_token=yxqqnn0900000001";
        }
    </script>
</head>
...
<div class="manage-header">
    <div class="container-fixed">
        ...
        <span style="line-height: 50px;font-size: 22px;">云就业企业校招管理平台</span>
        <span class="pull-right qq-help" style="margin: 10px 100px 0 0">
            <a target="_blank" href="https://wpa.qq.com/msgrd?v=3&uin=879656665&site=qq&menu=yes">
...
<div class="container">
        <div class="log-jumbotron" style="height: 1000px;">
            ...
            <div class="log-publicity">
                <h1 style="font-size: 40px;"><strong>全国首家“互联网+精准就业”平台</strong></h1>
            ...
            <p class="f18 mb18">企业登录<span style="color: orangered">(建议您使用谷歌浏览器)</span></p>

            <div class="text-wrap">
                <span class="icon icon-name"></span>
                <input type="text" placeholder="用户名（手机号码）" id="user_name" name="user_name">
            </div>
            <div class="text-wrap">
            ...
            <div class="text-wrap" id="div_code_input" style="width: 200px;float: left;display:none">
                <span class="icon icon-pass"></span>
                <input type="text" style="width: 150px" placeholder="验证码" id="valid_code" name="valid_code">
            </div>
...
<!--                 <a style="width:100px;padding: 0; class="btn-pr ok-btn f14 login-btn align-c" id="link_new_register">注册</a>
 -->                
 					<button type="button" id="btn_login" style="width:100px;padding: 0" class="btn-pr ok-btn f14 login-btn align-c">登录</button>
            </div>
            <p class="error-info color-red" id="error_info"></p>
            <p style="cursor: pointer;font-size: 12px;color: gray;" onclick="location.href='/login/forgetpassword'">忘记登录密码?</p>
        </div>
    </div>
</div>
</body>
...
<script>
    if(location.href.indexOf('hr.hunbys.com') > 0){
        ...
        $('#img_logo').attr('src', 'http://pic.bibibi.net/school/1463388776-6096.jpg');
        $('#img_bg').attr('src', 'http://pic.bibibi.net/public/images/bg/hunbys_bg.jpg');

        $('#link_new_register')
                .attr('href', 'http://hr.hunbys.com/login/schoolregister?school_token=yxqqnn0000000000')
                .attr('style', 'width:100px;padding: 0;');

    }
    var G_CONFIG = {};

    $.ajax({type : "get", url : "/login/gconfig", async : false,
    ...
    //注册按钮处理
    if($.getQueryStr('school_token') || location.href.indexOf('hr.hunbys.com') > 0){
    ...
    $('dd>a','#div_menu').each(function(){
        var url=$(this).attr('data-href');

        if(is_auth==1||url =='/setting/info'||url =='/user/bind'){
            $(this).attr('href',$(this).attr('data-href'));
        }
    }).on('click',function(){
        var url=$(this).attr('data-href');
        if(is_auth==0 && (url!='/setting/info' && url!='/gallery/index' && url!='/user/bind')){
            $('#tip_auth').show();
        }
    });
    ...
    seajs.use(_modulePath, function () {
        ...
        if(location.href.indexOf('/login/') < 0){
            if(!G_CONFIG.hr){
                var dialog = $.dialog({content: '登录超时', quickClose: true}).show();
                ...
                $('#btn_logout').click(function(){
                    $.get('/login/logout', function(data){
                        var dialog = $.dialog({content: '退出登录成功', quickClose: true}).show();
                        _timeout = setTimeout(function () {
                            dialog.close().remove();
                            location.href = login_url;
                        }, 2000);
            ...
            if(!!G_CONFIG.hr) {
                ...
                if(!!G_CONFIG.ticket){
                    ...
                    if(!!sso_url){
                        location.href = sso_url + (sso_url.indexOf('?')>0?'&':'?') + 'ticket='+ encodeURIComponent(G_CONFIG.ticket);
                        return;
                    }
                }
                var fair_id = $.getQueryStr('fair_id');
                if(!!fair_id){
                    var url_jobfair = '/campus/jobfair?school_token='+$.getQueryStr('school_token')+'&apply_type='+$.getQueryStr('apply_type')+'&fair_id='+fair_id;

                    var job_fair = $.getQueryStr('job_fair');
                    if(!!job_fair){
                        url_jobfair += '&job_fair='+encodeURIComponent(job_fair);
                    }

                    var meet_time = $.getQueryStr('meet_time');
                    if(!!meet_time){
                        url_jobfair += '&meet_time='+encodeURIComponent(meet_time);
                    }

                    var address = $.getQueryStr('address');
                    if(!!address){
                        url_jobfair += '&address='+encodeURIComponent(address);
                    }
            ...
            $.message({
                ...
                btns: [
                    ...
                    ['确定', function () {
                        var _this = this;
                        var postData = {};
                        if($.trim($("#pwd").val()).length > 0 && $.trim($("#pwd1").val()).length >0) {
                            postData.pwd = _security.hex_md5($("#pwd").val());
                            postData.pwd1 = _security.hex_md5($("#pwd1").val());
                            postData.pwd2 = _security.hex_md5($("#pwd2").val());
                            if (postData.pwd1 != postData.pwd2) {
                                $("#pwd2").val("").focus();
                                var dialog = $.dialog({content: '两次密码输入不一致', quickClose: true}).show();
                                _timeout = setTimeout(function () {
                                    dialog.close().remove();
                                }, 2000);
                                return false;
                            }else if(postData.pwd == postData.pwd1){
                                var dialog = $.dialog({content: '新密码和旧密码不能一样', quickClose: true}).show();
                                _timeout = setTimeout(function () {
                                    dialog.close().remove();
                                }, 2000);
                                return false;
